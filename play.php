...
function updateGame() {
    ...
    if (true) {
        if (!behaviour["block-move"]) {
            if (keys.right && !keys.left) {
                character.acc.x += character.behaviour.speed;
        ...
        if (keys.jump && !behaviour["block-jump"] && character.jumps < character.behaviour.jumps) {
            if (!keys.hasJump) {
                character.spd.y += -character.behaviour.jump.value;
    ...
    if (character.attacking) {
        if (behaviour) {
            if (behaviour.move ) {
                character.acc.x += behaviour.move.x;
                character.acc.y += behaviour.move.y;
            }
        }
    }
